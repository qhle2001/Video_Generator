...
def calculate_duration(model):
    print("The AI model is Calculating Duration of Word...")
    segments, info = model.transcribe("results/audio_file.mp3", word_timestamps=True)
...
def create_json(segments):
    ...
    with open('results/data.json', 'w', encoding='utf-8') as f:
        json.dump(wordlevel_info, f, ensure_ascii=False, indent=4)
...
def create_caption(textJSON, framesize,font = "Arial",color='white', highlight_color='yellow',stroke_color='black',stroke_width=1.5):
    ...
    fontsize = int(frame_height * 0.065) #6.5 percent of video height
...
def create_line_subtitles():
    model = download_modeling()

    segments = calculate_duration(model)

    wordlevel_info_modified = create_json(segments)
...
def customize_sub(videofilename):
    print("Video is Creating...")
    input_video = VideoFileClip(videofilename)
    frame_size = input_video.size

    linelevel_subtitles = create_line_subtitles()
    ...
    return input_video, all_linelevel_splits

def main():
    videofilename = "results/new_video.mp4"

    input_video, all_linelevel_splits = customize_sub(videofilename)
    ...
    final_video.write_videofile("results/output.mp4", fps=24, codec="libx264", audio_codec="aac")
